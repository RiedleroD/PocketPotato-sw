...
void setup() {
	...
	checkEEPROM();  // Check and load EEPROM values. Contrast after this function is set to last saved value or 255
}

void loop() {
  if (down.isClick() || down.isStep()) menu = constrain(menu + 1, 0, APPS - 1); // Down - next menu item
  else if (up.isClick() || up.isStep()) menu = constrain(menu - 1, 0, APPS - 1);  // Up - prev.
